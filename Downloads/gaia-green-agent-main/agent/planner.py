...
class Planner:
    ...
    The planner analyzes the problem statement using pattern matching and
    natural language processing to identify:
    1. What information needs to be extracted (numbers, values)
    2. What computations are required (mathematical operations)
    3. What tools should be used (calculator, etc.)
    4. The order of operations (single-step vs. multi-step)
    
    The planner supports:
    - Single-step arithmetic operations
    - Multi-step sequential operations (e.g., "X, then Y")
    - Word-based operators (plus, minus, times, divided by)
    - Explicit mathematical expressions
    
    Attributes:
        seed: Random seed for deterministic behavior (currently unused)
    
    Example:
        >>> planner = Planner(seed=42)
        >>> plan = planner.plan("Calculate 15 multiplied by 7, then add 23.")
        >>> len(plan['steps'])
        2
        >>> plan['requires_tools']
        True
    ...
    def __init__(self, seed: Optional[int] = None):
        """Initialize planner with optional seed for deterministic behavior.
        
        Args:
            seed: Random seed for deterministic behavior (currently unused,
                  reserved for future probabilistic planning features)
        """
        self.seed = seed
    ...
    def plan(self, problem: str) -> Dict[str, Any]:
        ...
        Analyzes the problem statement to identify mathematical operations,
        extract numbers, and determine the execution order. Supports both
        single-step and multi-step problems.
        
        Args:
            problem: The GAIA problem statement (non-empty string)
            
        Returns:
            Dictionary containing:
            - steps: List of ordered execution steps, each with:
              - step_id: Sequential step identifier
              - type: Step type ('computation', 'extraction', 'reasoning')
              - description: Human-readable step description
              - tool: Tool to use (e.g., 'calculator') if applicable
              - expression: Mathematical expression to evaluate
              - use_previous_result: Whether to use result from previous step
            - requires_tools: Boolean indicating if tools are needed
            - reasoning: Brief explanation of the plan
            - problem: Original problem statement
            
        Raises:
            ValueError: If problem is empty or None
