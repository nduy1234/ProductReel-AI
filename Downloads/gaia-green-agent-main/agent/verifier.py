...
class Verifier:
    ...
    Performs multi-level verification to ensure correctness and quality:
    1. Step-level verification: Checks each computation step
    2. Answer-level verification: Validates final answer format and sanity
    3. Retry logic: Determines when to retry failed steps
    
    The verifier ensures:
    - Mathematical correctness of computations
    - Consistency of intermediate results
    - Proper format of final answers
    ...
    Example:
        ...
        >>> step_result = {'success': True, 'result': {'result': '42', 'success': True}}
        >>> verification = verifier.verify_step(step_result)
        >>> verification['valid']
        True
    """
    
    def __init__(self, max_retries: int = 3):
        """Initialize verifier with retry configuration.
        
        Args:
            max_retries: Maximum number of retry attempts for failed verifications
                         (default: 3). Retries are attempted when verification
                         fails but the error is retryable.
        """
